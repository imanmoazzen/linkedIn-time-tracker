...
async function copyFiles() {
  const copyCssPromise = fs.readdir(SOURCE_DIRECTORY).then((filePaths) => {
    ...
    const copyPromises = cssPaths.map((filePath) =>
      fs.copyFile(path.join(SOURCE_DIRECTORY, filePath), path.join(DEV_DIRECTORY, path.basename(filePath)))
    );
    return Promise.all(copyPromises);
  });
  await Promise.all([
    copyCssPromise,
    fs.cp(path.join(SOURCE_DIRECTORY, "assets"), path.join(DEV_DIRECTORY, "assets"), { recursive: true }),
